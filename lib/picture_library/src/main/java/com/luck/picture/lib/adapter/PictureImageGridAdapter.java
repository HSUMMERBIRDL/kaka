...
public class PictureImageGridAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private boolean is_checked_num; // 两种模式 选择图片的时候左上角的圆圈不带数据 和 带数字（qq风格）
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if (getItemViewType(position) == TYPE_CAMERA) {
            ...
            if (selectMode == FunctionConfig.MODE_SINGLE) {  //  隐藏与显示右上角check
                contentHolder.ll_check.setVisibility(View.GONE);
            } else {
                contentHolder.ll_check.setVisibility(View.VISIBLE);
            }
            if (is_checked_num) {  // 在此种模式下  给选中的图片设置右上角数字
                notifyCheckChanged(contentHolder, image);
            }

            // TODO: 2017/3/13 0013  并没有从image的直接属性中去判断是否是选中的图片  也许是处于一些别的考虑 也许也是一个可以要优化的问题
            // 设置一些遮罩层
            ...
            //  如果允许预览 那么左上角和图片部分 要分别响应  不过不预览 两个响应是一样的
            //  所以这里是要处理一下在预览时左上角的处理
    ...
    private void changeCheckboxState(ViewHolder contentHolder, LocalMedia image) {
        ...
        // 图片大于等于选中的图片  该图片为被选中的话 弹出提示 return
        ...
        selectImage(contentHolder, !isChecked, true); // 被选中或取消 添加或去掉遮罩层
        if (imageSelectChangedListener != null) {
            imageSelectChangedListener.onChange(selectImages);
        }
    }
